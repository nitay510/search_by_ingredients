...
# — set up logging so we can see infos and warnings
log = logging.getLogger("diet")
log.setLevel(logging.INFO)
log.addHandler(logging.StreamHandler(stream=sys.stderr))

log.info("Loading spaCy …")
# — load small English model, but skip parser & ner for speed
NLP = spacy.load("en_core_web_sm", disable=["parser", "ner"])
KEEP_POS = {"NOUN", "PROPN"}    # — we only keep nouns & proper nouns

# — vegan rules: things that instantly fail or pass
...
# — keto rules: things that instantly pass or fail
...
# — regex pieces to remove quantities, units, punctuation
FRACTIONS   = "¼½¾⅓⅔⅛⅜⅝⅞"
NUM         = rf"\d+(?:[./]\d+)?|[{FRACTIONS}]"
UNITS       = ("cups?|cup|tbsp|tablespoons?|tbs|tsp|teaspoons?|"
               "lbs?|pounds?|oz|ounces?|grams?|kg|kilograms?|ml|l|"
               "liters?|pints?|pt|quarts?|qt|gal|gallons?|inch(?:es)?)")
...
REMOVE_RE   = re.compile(r"\b(?:fresh|large|medium|small|whole|"
                         r"boneless|skinless|washed|peeled|pitted|"
                         r"chopped|diced|sliced|shredded|grated|"
                         r"minced|crushed|trimmed|cleaned|piece|"
                         r"pieces|slice|slices|pinch|dash|taste|"
                         r"optional|bite|head|bay|leaf)\b", re.I)

# — turn raw text into a simple ascii, lowercase, no-units string
...
# — from that scrubbed string, keep only noun chunks up to 3 words
...
# — explode a CSV‐style ingredient cell into a Python list
    ...
    if txt.startswith("[") and txt.endswith("]"):
        ...
        if "," in txt:
            try:
                ...
                pass
    # 2) fallback: break on numbers, commas, newlines
    pieces = SPLIT_QTY.sub("\n", txt)
    parts  = [LEADING_QTY.sub("", p).strip()
              for p in re.split(r"[\n,]+", pieces)]
    return [p for p in parts if p]

# — load the two pre‐trained SVMs
...
def _main():
    ...
    # turn each recipe’s ingredients field into a Python list
    df["_ings"] = df["ingredients"].apply(to_list)

    # collect every unique raw ingredient string
    unique = {ing for row in df["_ings"] for ing in row}

    # preprocess all at once
    clean_map = {r: preprocess(r) for r in unique}

    # decide keto_map[r] for each raw ‘r’
    keto_map: dict[str,bool] = {}
    svmlist = []
    for r,c in clean_map.items():
        if any(tok in c for tok in KETO_ALLOW):
            ...
            svmlist.append((r,c))
    if KETO_MODEL and svmlist:
        raws, cleans = zip(*svmlist)
        for r,p in zip(raws, KETO_MODEL.predict(list(cleans))):
            keto_map[r] = bool(p)
    for r in unique:
        keto_map.setdefault(r, False)

    # same for vegan_map
    vegan_map: dict[str,bool] = {}
    svmlist = []
    for r,c in clean_map.items():
        toks = c.split()
        if toks and toks[0] in PLANT_BASES:
            ...
            svmlist.append((r,c))
    if VEGAN_MODEL and svmlist:
        raws, cleans = zip(*svmlist)
        for r,p in zip(raws, VEGAN_MODEL.predict(list(cleans))):
            vegan_map[r] = bool(p)
    for r in unique:
        vegan_map.setdefault(r, False)

    t0 = time()
    df["keto_pred"]  = df["_ings"].apply(lambda ings: all(keto_map[i] for i in ings))
    df["vegan_pred"] = df["_ings"].apply(lambda ings: all(vegan_map[i] for i in ings))
